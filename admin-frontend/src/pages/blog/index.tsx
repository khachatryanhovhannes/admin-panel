...
import {
  ...
  deleteBlogPage,
  deleteBlogPageContent,
  fetchAllBlogPages,
  fetchAllBlogPagesContent,
...
export default function BlogPagesManagement() {
  ...
  const handleAddPage = async () => {
    ...
    try {
      const createdPage = await createBlogPage({
        title: newTitle.trim(),
        slug: newSlug.trim(),
        isActive: true,
      });
  ...
  const handleDeletePageContent = async (pc: IPageContent) => {
    try {
      await deleteBlogPageContent(pc.id);
  ...
  const handleSavePageContent = async () => {
    ...
    try {
      if (currentContent.id) {
        const updated = await updateBlogPageContent(
          currentContent.id,
          newPayload
        );
        ...
        const created = await createBlogPageContent({
